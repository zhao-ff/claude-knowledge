...
启动 Query API 的 HTTP 服务。供 LLM Agent 或 Obsidian 插件远程调用。

```bash
node dist/index.js query-server -p 3457
# → http://localhost:3457
```

API 端点：
- `POST /api/query` — 提问，支持两种格式：
  ```json
  { "question": "transformer 是什么？" }
  { "messages": [{ "role": "user", "content": "..." }] }
  ```
- `GET /api/query?q=<question>` — 通过 URL 参数提问
- `GET /health` — 健康检查

返回格式：
```json
{
  "answer": "LLM 根据 Wiki 生成的答案...",
...
整个 `wiki/` 目录是纯 Markdown，可直接作为 Obsidian 仓库打开：

1. Obsidian 左下角 **打开其他仓库** → **打开文件夹** → 选择本项目的 `wiki/` 目录
2. Wiki 内 `[[page-name]]` 链接会自动渲染为可跳转的内部链接
3. 编译生成的概念页面、反向链接、文档索引在 Obsidian 图谱视图中完整可用

也可以在 Obsidian 中打开项目根目录，然后到 **设置 → 文件与链接** 中限制检测范围，避免 `.ts` 源码文件干扰视图。

#### Obsidian 插件

项目内置了一个 Obsidian 插件（`plugins/obsidian-api`），在 `wiki/` 仓库中已启用。

**功能：**

1. **向知识库提问** — 在 Obsidian 中直接按 `Ctrl/Cmd+P` → "向知识库提问"，弹窗中输入问题，答案会渲染为 Markdown，可一键保存为笔记。
2. **HTTP API** — 插件在本地启动 HTTP 服务（默认 `3458` 端口），供外部 LLM Agent 在 Obsidian 中创建和追加笔记：
   - `POST /api/new-note` — 创建笔记（含 `content` 和可选的 `filename`）
   - `POST /api/append-note` — 追加内容到已有笔记
   - `GET /health` — 健康检查

**使用前提：** 需要先启动 Query API 后端：

```bash
npm start query-server
```

插件会在 Obsidian 右下角提示服务状态（`Obsidian API: http://localhost:3458`）。在插件设置页（设置 → 插件选项 → Obsidian API Server）中可配置端口和后端地址。
